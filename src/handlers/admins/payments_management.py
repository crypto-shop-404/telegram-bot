...
@dp.message_handler(filters.Text('🌐 Coinbase'), is_user_in_db.IsUserInDB(), is_admin.IsUserAdmin())
...
@dp.callback_query_handler(callback_factories.PaymentSystemCallbackFactory().filter(
    system='coinbase', action='check'), is_user_in_db.IsUserInDB(), is_admin.IsUserAdmin()
)
...
@dp.callback_query_handler(callback_factories.PaymentSystemCallbackFactory().filter(
    system='coinbase', action='change_api_key'), is_user_in_db.IsUserInDB(), is_admin.IsUserAdmin()
)
...
@dp.message_handler(
    ...
    state=payments_management_states.ChangeCoinbaseData.waiting_api_key
)
